...
Controls a Viessmann heating system through the Home Assistant ViCare integration. Summer conditions select domestic-hot-water-only mode `dhw`. Away conditions outside summer select ViCare `ForcedReduced` (via Home Assistant HVAC mode `off`): heating remains at the boiler's configured reduced temperature and domestic hot water is disabled. Normal operation uses `dhwAndHeating`.
